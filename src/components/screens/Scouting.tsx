...
import { MapPin, Plus, Filter, Camera, Navigation, AlertTriangle, CheckCircle2 } from 'lucide-react';
...
const TASKS = [
  { id: 1, title: 'Проверить всходы', field: 'Поле 12-А', date: 'Сегодня', status: 'new', priority: 'high', author: 'Иванов И.' },
  { id: 2, title: 'Мониторинг сорняков', field: 'Поле 08-Б', date: 'Вчера', status: 'done', priority: 'medium', author: 'Петров П.' },
  { id: 3, title: 'Оценка болезней', field: 'Поле 03-В', date: '2 Июн', status: 'in-progress', priority: 'high', author: 'Сидоров С.' },
];
    ...
    <div className="h-full flex gap-6">
      ...
      <div className="w-1/3 flex flex-col gap-4">
         <div className="flex justify-between items-center">
           <h1 className="text-2xl font-bold text-slate-900">Скаутинг</h1>
           <Button size="sm" className="bg-[#2ECC71] hover:bg-emerald-600"><Plus className="w-4 h-4" /></Button>
         </div>

         <div className="flex gap-2">
           <Button variant="outline" size="sm" className="flex-1 bg-white">Мои задачи</Button>
           <Button variant="ghost" size="sm" className="flex-1 text-slate-500">Все задачи</Button>
         </div>

         <div className="space-y-3 overflow-y-auto flex-1 pr-2">
            {TASKS.map(task => (
              <Card key={task.id} className={`cursor-pointer hover:shadow-md transition-all p-4 border-l-4 ${
                task.priority === 'high' ? 'border-l-red-500' : 'border-l-blue-500'
              }`}>
                 <div className="flex justify-between items-start mb-2">
                   <h3 className="font-semibold text-slate-800">{task.title}</h3>
                   <Badge variant="outline" className="text-[10px]">{task.date}</Badge>
                 </div>
                 ...
                 <div className="flex justify-between items-center mt-2">
                   ...
                   <Badge className={`
                     ${task.status === 'new' ? 'bg-blue-100 text-blue-700 hover:bg-blue-100' : 
                       task.status === 'done' ? 'bg-green-100 text-green-700 hover:bg-green-100' : 
                       'bg-yellow-100 text-yellow-700 hover:bg-yellow-100'}
                   `}>
                     {task.status === 'new' ? 'Новая' : task.status === 'done' ? 'Выполнено' : 'В работе'}
                   </Badge>
                 </div>
              </Card>
            ))}
      ...
      <div className="flex-1 bg-slate-100 rounded-xl border border-slate-200 relative overflow-hidden">
         ...
         <div className="absolute bottom-4 right-4 w-80 bg-white rounded-lg shadow-xl p-4">
            ...
            <p className="text-xs text-slate-500 mb-3">Поле 12-А • 2 Июня 14:30</p>
            
            <div className="grid grid-cols-2 gap-2 mb-3">
               <div className="aspect-square bg-slate-100 rounded overflow-hidden">
                 <img src="https://images.unsplash.com/photo-1625246333195-78d9c38ad449?auto=format&fit=crop&q=80&w=200" className="w-full h-full object-cover" />
               </div>
               <div className="aspect-square bg-slate-100 rounded overflow-hidden">
                 <img src="https://images.unsplash.com/photo-1530836369250-ef72a3f5cda8?auto=format&fit=crop&q=80&w=200" className="w-full h-full object-cover" />
               </div>
            </div>

            <div className="text-sm text-slate-700 mb-3">
              Обнаружены очаги осота полевого. Требуется локальная обработка гербицидом.
            </div>

            <div className="flex gap-2">
               <Button size="sm" className="flex-1">Принять меры</Button>
